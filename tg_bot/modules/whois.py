...
def whois(bot: Bot, update: Update, args: List[str]):
    ...
    text += f"\n☣️Link akun: {mention_html(user.id, 'link🚪')}"

    num_chats = sql.get_user_num_chats(user.id)
    text += f"\n🌐Chat count: <code>{num_chats}</code>"
    text += "\n🎭Jumlah foto profil: {}".format(bot.get_user_profile_photos(user.id).total_count)
    ...
    if user.id == OWNER_ID:
        text += "\n🚶🏻‍♂️Waduh, orang ini adalah Boss saya🤴\nSaya mohon kalian jangan macam-macam!."
